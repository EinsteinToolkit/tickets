...
if [ -z "${OPENCL_DIR}" ]; then
        ...
        for libext in a so dylib; do
            ...
            for libdir in lib64 lib; do
                ...
                if [ -n "$OPENCL_DIR" ]; then
                    ...
                    OPENCL_LIB_DIRS=$OPENCL_DIR/$libdir
                    OPENCL_LIBS=OpenCL
                    break
                fi
            done
    ...
    # overwrite things for Mac since I have no idea how to autotest that there
    # and no Mac to test
...
# remove standard library locations
if [ -n "$OPENCL_DIR" ]; then
  if [ "X$OPENCL_DIR" = "X/usr" ]; then
    unset OPENCL_INC_DIRS
    unset OPENCL_LIB_DIRS
  fi
fi
