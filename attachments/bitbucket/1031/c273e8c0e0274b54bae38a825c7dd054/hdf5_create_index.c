...
#include <assert.h>
    ...
    CHECK_ERROR (attr_dataspace = H5Screate_simple (1, &rank, NULL));
    CHECK_ERROR (attr = H5Acreate (to, "h5shape", H5T_NATIVE_HSIZE,
                                            attr_dataspace, H5P_DEFAULT));
    CHECK_ERROR (H5Awrite (attr, H5T_NATIVE_HSIZE, &shape[0]));
    CHECK_ERROR (H5Aclose (attr));
    CHECK_ERROR (H5Sclose (attr_dataspace));
    }
#if 0
