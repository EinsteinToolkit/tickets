...
# Repeat a string once for each dimension, expanding 'I' to a
# different character (i,j,k,l) each time
sub rpt ($)
{
    my ($str) = @_;
    my @ret;
...
for $dim (1,2,3,4) {
    ...
    push @lines, map {expand $_} (
        ...
        (bsnl '    }'),
        (bsnl '    CCTK_LOOP[DIM](name##_interior,'),
        (bsnl '               '.(crpt '[C]').','),
        (bsnl '               '.(crpt '([C]blo)').','),
        ...
        (bsnl '        }'),
        (bsnl '        CCTK_LOOP[DIM](name##_boundaries,'),
        (bsnl '                   '.(crpt '[C]').','),
        (bsnl '                   '.(crpt 'lc_bmin[[I]]').','),
        (bsnl '                   '.(crpt 'lc_bmax[[I]]').','),
        (bsnlsep [rpt '                   lc_cctkGH->cctk_lsh[[I]]'], ',', ') {'),
