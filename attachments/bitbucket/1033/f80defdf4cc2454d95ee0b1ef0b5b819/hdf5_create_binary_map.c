...
int main (int argc, char *argv[])
{
  ...
  sscanf(argv[2], "checkpoint.chkpt.it_%*d.file_%d", &filenum);
  ...
  while (fgets (objectname, sizeof (objectname), listfile))
  {
    ...
    CopyObject (infile, objectname, &filenum);
  }
                          ...
                          void *_filenum)
{
  ...
  herr_t result;
  const int filenum = *(int*)_filenum;
...
#include <assert.h>
    ...
    // HACK: assumes that sizeof(int) is a a power of two
    len = (strlen(vname) + 1 + sizeof(int)-1) & ~(sizeof(int)-1);
    assert(len % sizeof(int) == 0);
    fwrite(vname, len, 1, stdout);
    len = (strlen(objectname) + 1 + sizeof(int)-1) & ~(sizeof(int)-1);
    assert(len % sizeof(int) == 0);
    fwrite(objectname, len, 1, stdout);
